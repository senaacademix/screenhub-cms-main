...
import { Screen, Publication, ContentItem } from "@/generated/prisma";
import { createScheduleSchema, CreateScheduleSchema } from "../schemas/schedule.schema";
import { addScheduleItemAction } from "../actions/schedule.actions";
...
export function CreateScheduleDialog({ screens, publishedPublications, defaultScreenId }: CreateScheduleDialogProps) {
  ...
  const form = useForm<CreateScheduleSchema>({
    ...
    defaultValues: {
      screenId: defaultScreenId || (screens[0]?.id || ""),
      publicationId: "",
      startTime: "00:00",
      endTime: "23:59",
      daysOfWeek: "ALL",
  ...
  const activePublication = publishedPublications.find((p) => p.id === selectedPubId);
  ...
  const onSubmit = async (values: CreateScheduleSchema) => {
    setIsPending(true);
    try {
      const res = await addScheduleItemAction(values);
      if (res.success) {
        toast.success("¡Programación de pantalla creada exitosamente!");
        form.reset();
        setSelectedDays(["ALL"]);
        setOpen(false);
      } else {
        toast.error(res.error || "No se pudo crear la programación");
      }
    } catch {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="rounded-xl font-bold bg-primary text-primary-foreground hover:bg-primary/90 shadow-lg shadow-primary/20 px-5 gap-2 h-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-xl font-black tracking-tight flex items-center gap-2">
                Estudio de Programación Global
              </DialogTitle>
              <DialogDescription className="text-xs text-muted-foreground">
                Asigna publicaciones aprobadas a pantallas y configura sus franjas horarias de emisión.
              </DialogDescription>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <Button 
              ...
              onClick={form.handleSubmit(onSubmit, (errors) => {
                ...
                toast.error(firstError || "Por favor selecciona la pantalla y la publicación");
              })} 
              disabled={isPending || !selectedPubId || !selectedScreenId}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-border/40 pb-4 mb-4 shrink-0">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-extrabold text-foreground uppercase tracking-wider">
                  1. Seleccionar Publicación Aprobada
                </h3>
              </div>
            ...
            <div className="flex-1 overflow-y-auto pr-1 space-y-3">
              {filteredPublications.length === 0 ? (
                <div className="h-64 flex flex-col items-center justify-center text-center p-8 border border-dashed border-border/60 rounded-2xl text-muted-foreground">
                  ...
                  <p className="text-sm font-bold text-foreground">No hay publicaciones aprobadas para este publicador.</p>
                  <p className="text-xs text-muted-foreground mt-1">Aprueba publicaciones en el módulo de Auditoría.</p>
                </div>
              ) : (
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5 overflow-y-auto pr-1">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-extrabold text-foreground uppercase tracking-wider">
                      2. Seleccionar Pantalla Objetivo
                    </h3>
                  </div>
                ...
                {activePublication && selectedScreenId && (
                  <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4 space-y-1.5 text-xs">
                    <div className="flex items-center gap-2 text-emerald-400 font-extrabold">
                      ...
                      <span>Resumen de Programación Lista</span>
                    </div>
                    <p className="text-foreground font-bold">
                      "{activePublication.title}" ➔ {screens.find((s) => s.id === selectedScreenId)?.name}
                    </p>
                    <p className="text-muted-foreground text-[11px]">
                      Emisión: <strong className="text-emerald-400 font-mono">{form.watch("startTime")} - {form.watch("endTime")}</strong> | Publicado por {activePublication.createdBy?.name || "Publicador"}
                    </p>
                  </div>
                )}
              </form>
